...
class User extends TrackStarActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('email, username, pass', 'required'),
			array('email, username, pass', 'length', 'max'=>256),
			// make sure each email address is unique
			array('email, username', 'unique'),
			array('pass', 'compare'),
			array('pass_repeat', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, email, username, pass, last_login_time, create_time, create_user_id, update_time, update_user_id', 'safe', 'on'=>'search'),
		);
	}
